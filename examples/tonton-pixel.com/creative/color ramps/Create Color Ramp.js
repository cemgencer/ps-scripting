...
function linear (points, position)
{
	var value;
	...
	{
		throw new Error ("linear: two points or more are required");
	}
	return value;
}
...
{
	var value = linear (points, position);
...
{
	return [ limit (red, 0, 255), limit (green, 0, 255), limit (blue, 0, 255) ];
}
...
function transformColor (rgb, hueShift, saturationMultiplier, brightnessMultiplier)
{
	...
	return matrixToVector (multiplyMatrix (multiplyMatrix (yuvToRgb, multiplyMatrix (hsb, rgbToYuv)), vectorToMatrix (rgb)));
}
...
function getParameters ()
{
	...
	//
	componentsOptions.formulaMenu = componentsOptions.topGroup.add ('dropdownlist', undefined, [ "RGB", "HSB", "HSL", "HCL" ]);
	componentsOptions.formulaMenu.alignment = [ "left", "center" ];
	componentsOptions.formulaMenu.helpTip = "Select the formula color model among: RGB, HSB, HSL, HCL (CIE-LCH).";
	...
	function evalComponentsFormula (index, formulas, colorModel, floatRange)
	{
		...
		{
			rgbColor[componentIndex] = Math.round (rgbColor[componentIndex]);
		}
		return rgbColor;
	}
	//
